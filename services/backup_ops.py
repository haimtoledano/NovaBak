...
def config_to_dict(config):
    return {
        ...
        "max_global_backups": config.max_global_backups or 4,
        "max_backups_per_host": config.max_backups_per_host or 2,
        "datastore_min_free_pct": config.datastore_min_free_pct or 10,
        "datastore_headroom_gb": config.datastore_headroom_gb or 20,
        "datastore_est_multiplier": config.datastore_est_multiplier or 1.1,
        "smtp_server": config.smtp_server or "",
        "smtp_port": config.smtp_port or 587,
        "smtp_user": config.smtp_user or "",
        "smtp_to_email": config.smtp_to_email or "",
        "smtp_use_tls": bool(config.smtp_use_tls) if config.smtp_use_tls is not None else True,
        "smtp_use_ssl": bool(config.smtp_use_ssl) if config.smtp_use_ssl is not None else False,
        "imap_server": config.imap_server or "",
